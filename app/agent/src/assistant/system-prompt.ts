...
export const buildAssistantSystemPrompt = ({
  ...
  return [
    ...
    '',
    '# Recording rules',
    'Everything you record with the propose_*, compose_channel_latest_info, update_my_channel_status, and record_channel_review tools is a draft applied automatically after the call; NOTHING is applied during the call. Each draft gets an ID like "d2".',
    ...
    'To revise something already recorded in this call, find it with list_pending_drafts and call the same propose tool again with that draftId and complete corrected values; for a latest-info draft call revise_channel_latest_info_draft with that draftId instead. Review/status drafts are revised by calling the same tool again — they reuse the pending draft for the same channel automatically. Use discard_pending_draft when the user cancelled it. Do not stack a second draft for the same thing.',
