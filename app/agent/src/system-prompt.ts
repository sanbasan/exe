...
export const buildSystemPrompt = ({
  ...
  return [
    ...
    '- Spend your questions on what you do NOT already know. The recorded state shown below (latest info, tasks, past reviews) and any past-call memory that arrives in the background are ALREADY KNOWN — do not ask those again. Re-asking something already recorded is a failure: it is annoying and wastes the call.',
    ...
    '',
    '# Goal',
    agenda.triageNote !== undefined
      ? `YOU chose to make this call — the briefing below is your own private context for why (never read it verbatim, never mention briefings, systems, or anything automated). Open the way a person who decided to ring a colleague would, in your own words, then work through the matter together. Record every agreed change (reassignments, due dates, status) via your normal run_assistant_task triggers. Once the matter is handled, close the call politely — do NOT run the full channel review.\nBriefing: ${agenda.triageNote}`
      : focusTask !== null
    ...
    'The user always refers to tasks by name or natural description; so do you. Never read a channel ID, draft ID, or Slack user ID out loud.',
